...
{
  //store pin number and set up matrix
  m_pin = pin;  // only digital pin 8 works right now!
...
void LEDMatrix::mapLEDXY(int y, int x, byte color[]) {
    //Extract colors from color array
    byte RED = color[0];
    byte GREEN = color[1];
    byte BLUE = color[2];

    //Account for "snake" pattern in the matrix
    int RGBlocation = 0;
...
void LEDMatrix::displayFrame(int hold){
    RGB_update(-1, 0, 0, 0);
    delay(hold);
    clearLEDs();
}
...
//This function is really nasty and I didn't write it, don't mess with it
void LEDMatrix:: RGB_update(int LED, byte RED, byte GREEN, byte BLUE){
